...
export default function PaginaDeDetalhe() {
    ...
    return (
        <div className='p-4'>
            ...
            {id && nome && preco ? (
                <p className={estiloDoDetalhe}>Exibindo detalhes para: ID {id},
                    Nome: {nome}, Preço: {preco}</p>
            ) : (
                <p className={estiloDoDetalhe}>Nenhum dado de produto
                    encontrado.</p>
            )}
            <button className={estiloDoBotao} onClick={() =>
                router.push('/pages/mestre')}>
